...
/* ----------------------------------------------------------------------
 Modified by EJR 03/11/16
  Put in full expressions for a_sq, a_pu, and added a_tw 
 Modified by EJR 04/27/16
  Multiplied a_sh by (r/2)^2 = (2*radi + h_sep)^2/4
 Modified by EJR 05/09/16
  Removed tw inclusion
  pair_lubricate_log_sh.cpp
 -------------------------------------------------------------------------*/
...
//            Modified by EJR 3/10/16
            a_sq = 6.0*MY_PI*mu*radi*(1.0/4.0/h_sep + 9.0/40.0*log(1.0/h_sep) + 3.0/112.0*h_sep*log(1.0/h_sep));
            
            a_sh = 6.0*MY_PI*mu*radi*(1.0/6.0*log(1.0/h_sep));
            a_sh = a_sh*radi*radi*(1.0 + h_sep + 1.0/4.0*h_sep*h_sep);
            
            a_pu = 8.0*MY_PI*mu*pow(radi,3.0)*(3.0/160.0*log(1.0/h_sep) + 63.0/4000.0*h_sep*log(1.0/h_sep));
            
//            a_tw = 1.0*MY_PI*mu*pow(radi,3.0)*h_sep*log(1.0/radi/h_sep);
        } else
...
//                tx += a_tw*wdotn*delx/r;
//                ty += a_tw*wdotn*dely/r;
//                tz += a_tw*wdotn*delz/r;
            //
            //-----------------------------
