...
/* ----------------------------------------------------------------------
 ...
 Modified by EJR 10/04/2018
 Added Bubble interaction
 Added Elastic Force
 -------------------------------------------------------------------------*/
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        r = sqrt(rsq);
          ...
          double delta_max = radi*0.05;
          double delta = (2.0*radi + h_sep - r)/2.0;
          
          if (delta >= delta_max) {
              delta = delta_max;
              r = 2.0*radi + h_sep - 2*delta;
              rsq = r*r;
          }
        ...
        // Elastic Force
          ...
          double a = sqrt(radi*(2*h_sep + abs(delta)));
          double E = 10.0*10.0;
        ...
        // BUBBLE
          ...
          a_sq = (2.0*log(1.0/h_sep)+4.0*(Euler+log(2.0)))*MY_PI*mu*radi;
