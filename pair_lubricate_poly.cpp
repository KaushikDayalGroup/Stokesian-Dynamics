    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (flaglog) {
          ...
          a_sh *= radi*radi*(1.0 + h_sep + 1.0/4.0*h_sep*h_sep);
            
          a_pu = (4.0-lambda*lambda)/80.0/(1.0-lambda)*log(1.0/h_sep);
          a_pu += (132.0-24.0*lambda-11.0*lambda*lambda-24.0*pow(lambda,3.0)-43.0*pow(lambda,4.0))/2000.0/(1.0-lambda)/(1.0-lambda)*h_sep*log(1.0/h_sep);
