...
/* ----------------------------------------------------------------------
 BUBBLE INTERACTION 10Aug15
 Modified by EJR 02/27/2019
  Made particle surface like bubbles
  Size difference is accounted for with an average between their sizes
 
 Modified by EJR 05/18/16
  Multiplied a_sh by (r/2)^2 = (2*radi + h_sep)^2/4
  Updated a_pu to match Ball and Mellrose g_11 + g_12
 
 pair_lubricate_poly_log_sh.cpp
------------------------------------------------------------------------- */
...
void PairLubricatePoly::compute(int eflag, int vflag)
{
  ...
  double rsq,r,h_sep,beta0,beta1,lambda,radi,radj;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        r = sqrt(rsq);
          ...
          //Reduced Radius - Davis et al 1989 and Kim 2005
          double rad_a = (radi*radj)/(radi+radj);
          
          // delta
          double delta_max = rad_a*0.05;
          double delta = (radi+radj + h_sep - r)/2.0;
          
          if (delta >= delta_max) {
              delta = delta_max;
              r = radi+radj + h_sep - 2*delta;
              rsq = r*r;
          }
        ...
        // Elastic Force *Need to account for particle sizes
          double a = sqrt(rad_a*(2*h_sep + abs(delta)));
          double E = 10.0*10.0;
          double Fela = a*E*delta;
        ...
        // scalar resistances
          ...
          // BUBBLE *Need to account for particle sizes
          double Euler = 0.577216;
          
          a_sq = (2.0*log(1.0/h_sep)+4.0*(Euler+log(2.0)))*MY_PI*mu*rad_a;
        ...
        // force due to Repulsion (only in normal direction)
